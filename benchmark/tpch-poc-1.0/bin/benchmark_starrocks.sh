...
QUERY_NUM=1
TRIES=3
touch result_starrocks.csv
truncate -s0 result_starrocks.csv
...
Total=0
echo -e "SQL\tTime(ms)" | tee -a result_starrocks.csv
while read -r query; do
    sync
    echo -ne "Q$QUERY_NUM\t" | tee -a result_starrocks.csv
    # warm up 3 times
    for i in {1..3}; do
    ...
    echo -n "${TRIES_TIME_AVG}" | tee -a result_starrocks.csv

    Total=`echo "${Total} + ${TRIES_TIME_AVG}" | bc`
    echo "" | tee -a result_starrocks.csv

    QUERY_NUM=$((QUERY_NUM + 1))
done < ${query_file}
echo -e "Total\t$Total" | tee -a result_starrocks.csv
