...
class StarRocksTableOperation(object):
    def __init__(self):
        self.lib = starrocks_lib.CommonLib()
    ...
    def create_db_table(self, data_dir_path):
        ...
        try:
            ...
            for sql_dict in ddl_sqls:
                ...
                if res is None:
                    print("%s. create table error" % (sql_dict["file_name"]))

                if not res["status"]:
                    print("%s. create table error, msg: %s" % (sql_dict["file_name"], res["msg"]))
                else:
                    print("%s executed successfully" % (sql_dict["file_name"]))
        finally:
    ...
    def flat_insert(self, data_dir_path):
        self.connect_starrocks()
        try:
            # use db
            starrocks_db = conf_parser.starrocks_db
            ...
            # check whether need to stream load lineorder table first
            res = self.lib.execute_sql("select count(*) from %s.lineorder" % conf_parser.starrocks_db, "dml")
            if res and res['result'][0][0] == 0:
                self.stream_load(data_dir_path)

            # get flat insert sql
            insert_sqls = self.lib.get_flat_insert_sqls()
            for sql_dict in insert_sqls:
                print("%s start" % (sql_dict["file_name"]))
                res = self.lib.execute_sql(sql_dict["sql"], "dml")
                if res is None:
                    print("%s. flat insert error" % (sql_dict["file_name"]))

                if not res["status"]:
                    print("%s. flat insert error, msg: %s" % (sql_dict["file_name"], res["msg"]))
                else:
                    print("%s success" % (sql_dict["file_name"]))
        finally:
            self.close_starrocks()
...
if __name__ == '__main__':
    starrocks_operation = StarRocksTableOperation()
    if len(sys.argv) < 2:
    ...
    if operation_type == "create":
        if len(sys.argv) < 3:
            ...
            sys.exit(-1)
        starrocks_operation.create_db_table(sys.argv[2])
    elif operation_type == "stream_load":
        if len(sys.argv) < 3:
            print("missing data dir path")
            sys.exit(-1)
        starrocks_operation.stream_load(sys.argv[2])
    elif operation_type == "flat_insert":
        starrocks_operation.flat_insert(sys.argv[2])
    elif operation_type == "broker_load":
        args = starrocks_operation.parse_broker_load_args()
        ...
        starrocks_operation.broker_load(table_name, hdfs_path, columns, columns_from_path)
    else:
